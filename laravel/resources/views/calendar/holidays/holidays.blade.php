...
<div class="calendar mx-auto">
        <div class="calendar-header">
            <button class="calendar-header__arrow mr-4" onclick="previous({{$month}}-1, {{$year}}, 'holidays')"><i class="fa fa-chevron-left"></i></button> 
            {{$miesiace[$month-1]}} {{$year}} 
            <button class="calendar-header__arrow ml-4" onclick="next({{$month}}+1, {{$year}}, 'holidays')"><i class="fa fa-chevron-right"></i></button>
        </div>
            ...
            @for($i=0; $i < ceil((cal_days_in_month(CAL_GREGORIAN, $month, $year) + date('N', strtotime($year."-".$month."-1")))/7)*7; $i++)
                ...
                @endif
                    ...
                    @if($z > 0 && $z <= cal_days_in_month(CAL_GREGORIAN, $month, $year))
                        ...
                        @foreach($holidays->where('date', '=', $year."-".$month."-$z") as $holiday)
                            <a href="{{ route('holiday', ['id' => $holiday->id]) }}" class="calendar-event calendar-event__conf" id="e_{{$holiday->id}}" onmouseover="hoverEvent(this.id)" onmouseout="hoverEvent(this.id)">
                                <span class="d-none d-sm-flex">{{ $holiday->name }}</span>
